...
`anytrain.idspace` 管 unified token id space：同一套 `IdSpace` 同时驱动多模态
embedding 和 tied head。它不训练 tokenizer，不组合多个 tokenizer，不迁移已有 tokenizer，
...
blocks。`IdSpaceEmbedding.as_head()` 用它构造 compact head，避免在 embedding 层重复维护
special hole 规则。
...
`IdSpaceEmbedding.as_head()` 返回的私有 head view 和 `IdSpaceEmbedding` 共享同一批权重；它是 compact view，输出列从 0 开始，
...
`IdSpaceEmbedding.as_head()` 创建，不作为公开类导出。下游如果要算
