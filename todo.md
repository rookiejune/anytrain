...
- `IntBPE.repeat_interleave` 增加 batch packed 输出形态：返回 concat 后的 `expanded_x`、`expanded_unit_ids` 和 `offsets`。
- `IntBPE.repeat_interleave` 增加 batch padded 输出形态：返回 `padded_x`、`padded_unit_ids` 和 `mask`。
